---
name: "tim-browser"
description: "Browser automation for agents using Vibium (WebDriver BiDi) with assertions, batch execution, visual diff, prompt-injection scanning, and semantic intents. Use when any skill needs to drive a real browser — visual testing, accessibility audits, E2E flow verification, pentest validation, or exploratory testing."
---

# TIM Browser

Thin wrapper around [Vibium](https://github.com/VibiumDev/vibium) that adds QE-specific primitives: typed assertions, batch execution, visual-diff against baselines, prompt-injection scanning, and semantic intent scoring.
...
- When a skill needs to navigate, read, interact with, or capture a web page
...
node .claude/skills/tim-browser/scripts/assert.js --checks '[
...
## Primitives (this skill's value-add)

All scripts live in `.claude/skills/tim-browser/scripts/` and shell out to `vibium`. They expect `vibium` to be on PATH.
...
Uses `pixelmatch` for pixel comparison; returns similarity % and diff image path.
...
vibium wait url "/dashboard"
vibium storage -o .tim/auth/myapp.json

# Every subsequent run
vibium storage restore .tim/auth/myapp.json
...
If `vibium` is not installed, the qe-browser helper scripts implement the contract automatically. **You don't have to grep error strings.** Each script:
...
case $EXIT in
  ...
  2) echo "skipped — vibium not installed; run \`npm install -g vibium\`" ;;
esac
```
...
```json
{
  ...
  "output": {
    "operation": "assert",
    "summary": "vibium binary not found on PATH. Install via `npm install -g vibium`.",
