// Shared helpers for tim-browser scripts.
...
//   - Matches how other TIM skills (eg. a11y-ally) invoke external tools
...
// Inject `--headless` into every vibium call by default. The tim-browser
...
//
// Opt out by setting TIM_BROWSER_HEADED=1 in the environment.
function injectHeadless(args) {
  if (process.env.TIM_BROWSER_HEADED === '1') return args;
...
function vibium(args, { input, timeoutMs = 30000 } = {}) {
  ...
  if (result.error && result.error.code === 'ENOENT') {
    throw new VibiumUnavailableError(
      'vibium binary not found on PATH. Install via `npm install -g vibium`.'
    );
  }
  ...
  return envelope({
    ...
    details: {
      ...
      remediation: [
        ...
        'Set TIM_BROWSER_HEADED=1 only for interactive debugging (not the cause here)',
      ],
    },
