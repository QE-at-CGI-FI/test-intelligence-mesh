...
priority: high
tokenEstimate: 1100
agents: [visual-tester, test-generator, quality-gate, accessibility-auditor]
...
const a11yFleet = await FleetManager.coordinate({
  ...
  agents: [
    'visual-tester',     // Visual & keyboard checks
    'test-generator',    // Generate a11y tests
    'quality-gate'       // Enforce compliance
  ],
  topology: 'parallel'
});
```
...
const a11yFleet = await FleetManager.coordinate({
  ...
  agents: [
    'visual-tester',   // axe-core, keyboard, focus
    'test-generator',  // Generate a11y test cases
    'quality-gate'     // Block non-compliant builds
  ],
  topology: 'parallel'
});
```
