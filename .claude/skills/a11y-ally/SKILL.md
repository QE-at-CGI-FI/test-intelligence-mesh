...
Uses the **tim-browser** fleet skill as the browser engine. tim-browser wraps Vibium (WebDriver BiDi, 10MB Go binary) and provides the browser primitives we rely on. See `.claude/skills/tim-browser/SKILL.md`.

### 1.1: PRIMARY — tim-browser via Vibium CLI
```bash
# Navigate
...
If Vibium MCP tools are registered (`mcp__vibium__*`), prefer them; otherwise shell out to the `vibium` binary if.

### 1.2: Run axe-core + WCAG assertions via tim-browser
```bash
  ...
  try {
    ...
    try {
      const cookieSelectors = [
        'button:has-text("Accept")', 'button:has-text("Akzeptieren")', 'button:has-tet("Hyväksy evästeet")',
...
## STEP 9: SCREEN READER TESTING GUIDE

Manual testing instructions (cannot be fully automated):

### 9.1: NVDA (Windows - Free)
```
...
### 9.4: Screen Reader Testing Checklist
